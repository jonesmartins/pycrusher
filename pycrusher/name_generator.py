...
def check_filenames(input_name: str, output_name: str) -> tuple[str, str]:
    ...
    if not output_name:
        ...
        """
    acceptable = set([".bmp", ".jpg", ".pbm", ".pgm", ".png", ".ppm", ".rgb", ".webp"])
...
def get_final_output_name(
    ...
    Returns:
        ...
        configuration.
    """
    typeless_output, file_format = check_filenames(input_name, output_name)
    if not output_name:
        typeless_output = "{}_i{}e{}".format(typeless_output, iterations, extra)
        if reverse:
            typeless_output += "r"
        if preprocess:
            typeless_output += "p"
        if color != [1.0] or color != 1.0:
            typeless_output += "c{}".format(str(color).replace(" ", ""))
